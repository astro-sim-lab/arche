...
def _prim_label(d: dict) -> str:
    ...
    zs = (
        r"\zeta_0 = 0"
        if zeta0 == 0.0
        else rf"\zeta_0 = {zeta0:.3e}\ \mathrm{{s}}^{{-1}}"
    )
    fr_part = rf",\ f_{{\mathrm{{ret}}}} = {f_ret:g}" if f_ret != 1.0 else ""
...
def _metal_label(d: dict) -> str:
    ...
    f_ret = d.get("f_ret", 1.0)
    zs = r"0" if zeta0 == 0.0 else rf"{zeta0:.3e}"
    Zs = r"0" if Z_metal == 0.0 else rf"{Z_metal:.3e}"
    base = rf"\zeta_0={zs}\ \mathrm{{s}}^{{-1}},\ Z/Z_{{\odot}}={Zs}"
    if f_ret != 1.0:
        base += rf",\ f_{{\mathrm{{ret}}}} = {f_ret:g}"
...
def _draw_phase(ax, d: dict, is_metal: bool):
    ...
    if is_metal and "T_gr_K" in d:
        ax.plot(
            d["nH"], d["T_gr_K"], lw=1.2, ls="--", color="tab:orange", label="grain $T$"
        )
    ax.set_xscale("log")
    ax.set_yscale("log")
...
def _draw_cooling(ax, d: dict, is_metal: bool):
    ...
    for key, label, color, ls in components:
        ...
        if np.any(np.isfinite(neg)):
            ax.plot(
                nH, neg, color=color, ls="--", lw=0.7, alpha=0.5, label=f"{label} (−)"
            )
    ax.set_xscale("log")
    ax.set_yscale("log")
...
def _phase_cooling(d: dict, label: str, tag_sfx: str, save_dir: str, is_metal: bool):
    fig, (ax_phase, ax_cool) = plt.subplots(
        2, 1, figsize=(7, 10), sharex=True, gridspec_kw={"hspace": 0.05}
    )
    ...
    ax_phase.set_title(
        rf"{prefix} collapse: $n_\mathrm{{H}}$–$T$ and cooling/heating"
        f"\n[{label}]",
        fontsize=11,
    )
...
def _plot_jeans(d: dict, label: str, tag_sfx: str, save_dir: str):
    fig, (ax1, ax2, ax3) = plt.subplots(
        3, 1, figsize=(7, 11), sharex=True, gridspec_kw={"hspace": 0.05}
    )
    nH = d["nH"]
    MJ = d["M_J"] / M_SUN
    tff = d["t_ff"] / YR
    ...
    ax1.axhline(1.0, color="gray", lw=0.8, ls="--", alpha=0.6, label=r"$1\,M_\odot$")
    ax1.set_yscale("log")
...
def _plot_thermal_balance(d: dict, label: str, tag_sfx: str, save_dir: str):
    fig, axes = plt.subplots(
        3, 1, figsize=(7, 10), sharex=True, gridspec_kw={"hspace": 0.05}
    )
    ...
    ax_net.set_ylabel(r"$|\Lambda_\mathrm{net}|$  [erg g$^{-1}$ s$^{-1}$]", fontsize=11)
    ...
    ax_cr.set_ylabel(r"$\Gamma_\mathrm{CR}/|\Lambda_\mathrm{net}|$", fontsize=11)
    ...
    ax_ratio.plot(nH, ratio_cool, lw=1.4, label=r"$t_\mathrm{cool}/t_\mathrm{ff}$")
    if "t_chem" in d:
...
def _plot_summary(d: dict, tag_sfx: str, save_dir: str, is_metal: bool):
    fig, axes = plt.subplots(
        2, 2, figsize=(13, 9), gridspec_kw={"hspace": 0.30, "wspace": 0.32}
    )
    ...
    _draw_phase(ax_phase, d, is_metal)
    prefix = "Metal-grain" if is_metal else "Zero-metal"
    ax_phase.set_title(rf"{prefix}: $n_\mathrm{{H}}$–$T$", fontsize=10)
    ...
    if is_metal:
        ...
        for sp, ylabel in _METAL_SPECIES:
            ...
            if idx is not None:
                ...
                ax_sp.plot(d["nH"], d["y"][:, idx], lw=1.3, color=color, label=sp)
        ax_sp.legend(fontsize=8, ncol=3)
    else:
        for sp, lbl in [("H", r"H"), ("H2", r"H$_2$"), ("e-", r"$e^-$"), ("HD", r"HD")]:
            idx = get_species_idx(d, sp)
            if idx is not None:
    ...
    ax_diag.plot(
        nH, ratio_cool, lw=1.4, color=c_cool, label=r"$t_\mathrm{cool}/t_\mathrm{ff}$"
    )
    ax_diag.plot(
        nH,
        ratio_chem,
    ...
    ax_diag.set_xlim(1e-1, 1e23)
    ax_diag.set_ylim(1e-3, 1e7)
    ax_diag.grid(True, which="both", alpha=0.3)
    ax_diag.set_title(
        r"Characteristic timescales / $t_\mathrm{ff}$  and  $M_\mathrm{J}$", fontsize=10
    )

    ax_mj = ax_diag.twinx()
    ax_mj.plot(nH, MJ, lw=1.4, color=c_mj, ls="-.", label=r"$M_\mathrm{J}$")
    ax_mj.axhline(1.0, color=c_mj, lw=0.7, ls=":", alpha=0.5)
    ax_mj.set_yscale("log")
    ax_mj.set_ylabel(r"$M_\mathrm{J}$  [$M_\odot$]", fontsize=10, color=c_mj)
...
def main():
    ...
    parser.add_argument(
        "--h5dir", default=None, help="Directory with collapse_CR<tag>.h5 (prim)"
    )
    parser.add_argument(
        "--cr-tag",
        default=None,
    ...
    parser.add_argument(
        "--save", default="results/prim/fig", help="Output directory for prim PNG files"
    )
    parser.add_argument(
        "--metal-h5dir",
        default=None,
    ...
    if args.h5dir is not None:
        ...
        print(
            f"  zeta0={zeta0:.3e} s^-1  f_ret={f_ret}"
    ...
    if args.metal_h5dir is not None:
        ...
        jlw21 = float(d.get("J_LW21", 0.0))
        print(
            f"  zeta0={zeta0:.3e} s^-1  Z_metal={Z_metal:.3e}  f_ret={f_ret}"
            + (f"  J_LW21={jlw21:.3g}" if jlw21 > 0.0 else "")
            + (f"  z={zred_tag}" if zred_tag else "")
            + f"  ({len(d['nH'])} rows)"
        )
        metal_sfx = (
            f"CR{cr_tag}_Z{z_tag}"
            + (f"_fret{fret_tag}" if fret_tag else "")
            + (f"_JLW{jlw_tag}" if jlw_tag else "")
            + (f"_z{zred_tag}" if zred_tag else "")
        )

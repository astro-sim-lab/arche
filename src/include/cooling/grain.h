...
//
// Provides grain temperature, Planck-mean opacity, grain volume, vaporisation
// temperature, and photoelectric heating (grtemp, kp_gr, vol_gr, vaptemp,
// phelectr).
//
...
struct GrainState {
  ...
  double T_gr_K =
      std::numeric_limits<double>::quiet_NaN();  // grain temperature [K]
  double k_gr = 0.0;     // Z-weighted Planck opacity [cm^2/g]
...
// ---------------------------------------------------------------------------
inline void vaptemp(double rho_in, double& T_ice, double& T_vo, double& T_ro,
                    double& T_tr, double& T_ir, double& T_pyr, double& T_ol) {
  // log-spaced density nodes [g/cm^3]
  static constexpr double ro[8] = {1.0e-18, 1.0e-16, 1.0e-14, 1.0e-12,
                                   1.0e-10, 1.0e-08, 1.0e-06, 1.0e-04};
  // tt[col][row]: 4 columns = (water ice, metallic iron, orthopyroxene,
  // olivine) 8 rows = density nodes
  ...
  // Interpolation in log10(rho) space (nodes are log-spaced: 1e-18..1e-4, step
  // 2 dex)
...
// ---------------------------------------------------------------------------
// kp_gr — Planck mean mass absorption coefficient [cm^2/g] for solar-Z dust
// Semenov et al. 2003; 5 polynomial segments blended with tanh
// ---------------------------------------------------------------------------
inline double kp_gr(double rho, double T_in) {
  ...
  static constexpr double eP[5][6] = {
      ...
      {-0.664969066656727e-10, 0.427373694560880e-07, -0.108371821805323e-04,
       0.130732588474620e-02, -0.191556936842122e-02, 0.423062838413742e-03},
      // Region 2
      {-0.371134628305626e-11, 0.472321713029246e-08, -0.210704968520099e-05,
       0.311604311624702e-03, 0.302186734201724e-01, -0.173587657890234e+01},
      // Region 3
      {0.233228177925061e-12, -0.681138400996940e-09, 0.784339065020565e-06,
       -0.436299138970822e-03, 0.120954274022502e+00, -0.638046050114383e+01},
      // Region 4
      {0.115933380913977e-12, -0.349467552126090e-09, 0.417313950448598e-06,
       -0.240971082612604e-03, 0.664915248499724e-01, -0.345042341508906e+01},
      // Region 5 (highest T)
      {-0.503398974713655e-15, 0.417753047583439e-11, -0.140583908595144e-07,
       0.238329845360675e-04, -0.166789974601131e-01, 0.667823366719512e+01}};
  ...
  for (int s = 0; s < 5; ++s) {
    k_rxn[s] =
        ((((eP[s][0] * T_in + eP[s][1]) * T_in + eP[s][2]) * T_in + eP[s][3]) *
             T_in +
         eP[s][4]) *
            T_in +
        eP[s][5];
  ...
  return (k_rxn[0] - k_rxn[1]) * step(T_in, Tv[0]) +
         (k_rxn[1] - k_rxn[2]) * step(T_in, Tv[1]) +
         (k_rxn[2] - k_rxn[3]) * step(T_in, Tv[2]) +
         (k_rxn[3] - k_rxn[4]) * step(T_in, Tv[3]) +
         k_rxn[4] * step(T_in, Tv[4]);
}
...
inline double vol_gr(double rho, double T_K) {
  ...
  return (0.78e-4 - 0.46e-4 * x(T_K, T_tr)) * x(T_K, T_ir) +
         7.19e-4 * x(T_K, T_ol) + 2.16e-4 * x(T_K, T_pyr) +
         1.18e-4 * x(T_K, T_tr) + 23.53e-4 * x(T_K, T_ro) +
         6.02e-4 * x(T_K, T_vo) + 12.93e-4 * x(T_K, T_ice);
}
...
//   func(Td) = kp_gr(rho,Td)*Td^4*esc_cnt
//            - nH * funcL(Td)
//            - kp_gr(rho,Td)*(T_gas^4*esc_fact + T_rad^4*exp(-tau))
...
// ---------------------------------------------------------------------------
inline void grtemp(double nH, double T_gas, double tau_cnt, double esc_cnt,
                   double esc_fact, double T_rad, double& T_gr) {
  constexpr double yHe = abundance_ref::yHe;
  constexpr double m_p = phys::m_p;
  const double rho = (1.0 + 4.0 * yHe) * m_p * nH;

  auto funcL = [&](double Td) {
    return 4.9e-2 * detail::vol_gr(rho, Td) * std::sqrt(T_gas * 1.0e-3) *
           (1.0 - 0.8 * std::exp(-75.0 / T_gas)) * (T_gas - Td);
  };
  auto func = [&](double Td) {
    double kp = detail::kp_gr(rho, Td);
    return kp * std::pow(Td, 4.0) * esc_cnt - nH * funcL(Td) -
  ...
  if (f0 == 0.0) {
    T_gr = T_gas;
    return;
  }  // dust evaporated: kp≈0,vol_gr≈0 → T_gr=T_gas
...
// x   = G_0 * sqrt(T_K) / (y_e * nH)
// eps = epsilon efficiency
// ---------------------------------------------------------------------------
inline void phelectr(double nH, double T_K, double y_e, double Z_metal,
                     double G_0, double A_v, double& Gam_pe,
                     double& Lambda_pe) {
  constexpr double yHe = abundance_ref::yHe;
  constexpr double m_p = phys::m_p;
  const double rho = (1.0 + 4.0 * yHe) * m_p * nH;
  ...
  double D = (vol0 > 0.0) ? detail::vol_gr(rho, T_K) / vol0 * Z_metal : 0.0;

  double x = G_0 * std::sqrt(T_K) / (y_e * nH + 1.0e-30);
  ...
  Lambda_pe = 2.78e-6 * std::pow(T_K, 0.944) * std::pow(x, beta) * (y_e * nH) *
              D * norm;
}

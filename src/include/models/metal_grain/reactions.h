...
inline void init_topology(MetalGrainTable& tbl) {
  ...
  // Reaction 273 (He+ + CH4 -> H2 + He + CH+ + H) stores only three of its four
  // products; rate_kernel.h injects the H at run time.  Declare it so
  // build_invariants() balances the same reaction the kernel integrates.
  tbl.extra_product_rxn = special::rxn_4body_H;
  tbl.extra_product_sp = static_cast<int>(Sp::H);
}

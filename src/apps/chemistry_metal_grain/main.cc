...
//   - supply all column densities from column integration (TreeCol, Sobolev,
//   etc.)
...
//   METAL_CHEM_TABLE — path to metal_grain.h5 chemistry table  (optional,
//                       default: compile-time METAL_CHEM_TABLE)
//   METAL_XNH       — H number density [cm^-3]        (optional, default: 1e4)
//   METAL_T_K       — gas temperature [K]              (optional, default:
//   100.0) METAL_YE0       — initial electron / H+ fraction  (optional,
//   default: 1e-4) METAL_YH2       — initial H2 fraction (optional, default:
//   6e-7) METAL_YHD       — initial HD fraction              (optional,
//   default: 4e-10) METAL_NSTEPS    — number of integration steps (optional,
//   default: 200) METAL_DT        — time step [s]                    (optional,
//   default: 1e10) METAL_CR_ATTEN_COL_DENS   — CR attenuation column density [g
//   cm^-2]
...
//                                (optional, default: 0.0, must be >= 0)
//   METAL_C_GAS_FRAC          — initial C gas fraction (optional, default:
//   0.28, [0,1]) METAL_O_GAS_FRAC          — initial O gas fraction (optional,
//   default: 0.54, [0,1]) METAL_MG_GAS_FRAC         — initial Mg gas
//   fraction(optional, default: 0.02, [0,1]) METAL_T_CR_DES            —
//   effective CR desorption spike temperature [K]
//                                (optional, default: 70.0, must be > 0)
//   METAL_REDSHIFT            — cosmological redshift z (optional, default:
//   0.0) METAL_JLW21               — LW intensity J_21       (optional,
//   default: 0.0) METAL_ABUNDANCE_SET       — abundance preset name (optional,
//   default: solar)
...
int main() {
  ...
  const char* env_table = std::getenv("METAL_CHEM_TABLE");
  const std::string chem_table =
      (env_table && env_table[0] != '\0') ? env_table : METAL_CHEM_TABLE;

  // ── Required: METAL_ZETA0 ─────────────────────────────────────────────────
  const char* env_zeta0 = std::getenv("METAL_ZETA0");
  if (!env_zeta0 || env_zeta0[0] == '\0') {
    std::fprintf(
        stderr, "ERROR: METAL_ZETA0 is required. Example: METAL_ZETA0=1e-17\n");
    return 1;
  }
  double zeta0 = 0.0;
  try {
  ...
  arche::MetalTablePtr tbl = arche::load_metal_table_owned(chem_table);
  ...
  arche::MetalCellPtr cell = arche::metal_cell_create_owned();
  arche::ChemStateMG& st = arche::metal_cell_state(*cell);
  auto& y = st.y;
  ...
  // ── Print header ──────────────────────────────────────────────────────────
  std::printf(
      "# metal-grain chemistry demo:"
      " nH=%g cm^-3  T_K=%g K  Z=%g Zsun  zeta0=%g s^-1"
      ...
      nH, T_K, Z_metal, zeta0, y_Hp, y_H2, y_HD, cr_atten_col_dens,
      cr_atten_second_frac, cr_metal_bkgnd, sra_rate, lra_rate, c_gas_frac,
      o_gas_frac, mg_gas_frac, t_cr_desorp, abund.name, zred, params.T_rad,
      jlw21);
  std::printf(
      "# %5s  %10s  %11s  %11s  %11s  %10s"
      "  %11s  %11s  %11s  %11s\n",
      "step", "t [s]", "y[H]", "y[H2]", "y[CO]", "T_gr[K]", "L_line", "L_cnt",
      "L_gr", "Gam_CR");
  std::printf(
      "# %5s  %10s  %11s  %11s  %11s  %10s"
      "  %11s  %11s  %11s  %11s\n",
  ...
  for (int it = 1; it <= nstep; ++it) {
    ...
    auto Nc = [&](int idx, double mass_amu) -> double {
      ...
      const double lsh =
          std::min(lmbd_J, 6.0 * vD * 1.0e13);  // 1e13 s ~ t_Hubble
      return y[idx] * nH * lsh;
    };
    ...
    shield.Nc_H = Nc(0, 1.0);
    shield.Nc_H2 = Nc(1, 2.0);
    shield.Nc_HD = Nc(12, 3.0);
    shield.Nc_CO = Nc(32, 28.0);
    shield.Nc_OH = Nc(31, 17.0);
    shield.Nc_H2O = Nc(33, 18.0);
    shield.Nc_CII = Nc(22, 12.0);
    shield.Nc_CI = Nc(16, 12.0);
    shield.Nc_OI = Nc(29, 16.0);
    shield.J_LW21 = jlw21;
    ...
    const auto rates =
        arche::chem_full_step_metal(*cell, dt, params, shield, *tbl);

    mu = st.mu;
    gamma = st.gamma;
    k_gr = rates.k_gr;
    k_gas = rates.k_gas;
    // Grain temperature is now returned (not written into params); feed it
    // back as the next step's warm-start seed.
    params.T_gr_K = rates.T_gr_K;

    // Print
    std::printf(
        "  %5d  %10.3E  %11.4E  %11.4E  %11.4E  %10.3E"
        "  %11.4E  %11.4E  %11.4E  %11.4E\n",
        it, t, y[0], y[1], y[32], params.T_gr_K, rates.Lambda_line,
        rates.Lambda_cnt, rates.Lambda_gr, rates.Gamma_CR);
